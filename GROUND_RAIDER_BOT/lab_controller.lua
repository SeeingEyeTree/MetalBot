...
-- GROUND_RAIDER_BOT: the vehicle plant makes Incisors (corgator) and the air lab makes
-- fighters (corveng) to escort them; a lab builds only what it can, so one list serves both.
local RAID_CAP   = { corgator = 16, corveng = 4 }   -- alive at once, per unit
local RAID_NAMES = RAID_CAP
local RAID_CONT  = { "corgator", "corveng" }        -- continuing production
local RAID_TARGET = 999  -- unused here: RAID_CAP limits each unit instead
local raidAliveBy = {}   -- [unitName] = how many are alive
...
function widget:Initialize()
    ...
    local okB, bp = pcall(VFS.Include, "LuaUI/Widgets/blueprints/general/ground_raider_blueprint.lua")
    if okB and type(bp) == "table" and bp.units then
        ...
        Spring.Echo("[LabCtrl] no opening list in ground_raider_blueprint.lua: " .. tostring(bp))
    end
...
function widget:UnitFinished(unitID, unitDefID, teamID)
    ...
    if RAID_NAMES[d.name] then
        ...
        raidAliveBy[d.name] = (raidAliveBy[d.name] or 0) + 1
        return
    end
...
function widget:UnitDestroyed(unitID, unitDefID, teamID)
    ...
    if ud and RAID_NAMES[ud.name] then
        ...
        raidAliveBy[ud.name] = math.max(0, (raidAliveBy[ud.name] or 0) - 1)
    end
    labs[unitID]        = nil
...
function widget:GameFrame(frame)
    ...
    if #openingDefs > 0 then
        for labID, labDefID in pairs(labs) do
            if not raidPrimed[labID] and spGetUnitDefID(labID) and UnitDefs[labDefID].buildOptions then
                -- Only the units THIS lab can build, in the blueprint's order: the vehicle
                -- plant takes the Incisors, the air lab the fighters.
                local mine = {}
                for _, defID in ipairs(openingDefs) do
                    if CanBuild(labDefID, defID) then mine[#mine + 1] = defID end
                end
                if #mine > 0 then
                    raidPrimed[labID] = true
                    -- Plain appends, in order.  The macro controller queues nothing in a
                    -- raid lab until the grid hand-off, so the list IS the front of the queue.
                    for _, defID in ipairs(mine) do
                        spGiveOrderToUnit(labID, -defID, {}, {})
                    end
                    Spring.Echo(string.format("[LabCtrl] opening: %d units queued in %s %d",
                        #mine, UnitDefs[labDefID].name, labID))
                end
            end
        end
    end
    ...
    for labID, labDefID in pairs(labs) do
        if spGetUnitDefID(labID) and QueueEmpty(labID) then
            ...
            -- Continuing production: what THIS lab can build and is not already at its cap.
            local mine = {}
            if UnitDefs[labDefID] and UnitDefs[labDefID].buildOptions then
                for _, defID in ipairs(contDefs) do
                    local nm = UnitDefs[defID].name
                    if CanBuild(labDefID, defID) and (raidAliveBy[nm] or 0) < (RAID_CAP[nm] or 0) then
                        mine[#mine + 1] = defID
                    end
                end
            end
            local raidLab = #mine > 0
            if raidLab then
                -- A batch, not one: the lab's own build power is the bottleneck.
                for _ = 1, RAID_BATCH do
                    contIdx = contIdx % #mine + 1
                    spGiveOrderToUnit(labID, -mine[contIdx], {}, {})
                end
            elseif picks then
