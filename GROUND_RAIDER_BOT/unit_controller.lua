...
-- ── Ground raid (GROUND_RAIDER_BOT) ────────────────────────────────────────────
-- Incisors (corgator) gather at a rally point beyond the factory's exit corridor, go as a
-- wave, and attack-move to the enemy base; fighters (corveng) GUARD them, because
-- Shuriken stun ground units and fighters are what stops them.  Everything is echoed as
-- [RAID] lines, including "STUCK" when a unit never leaves the base: a ground factory
-- with no way out was the main risk of this bot.

local RAID_DEFS = { corgator = "ground", corveng = "escort" }
local RAID_WAVE_GROUND = 4      -- launch once this many Incisors wait at the rally point...
local RAID_FORCE_FRAME = 9000   -- ...or at 5:00 with at least 2, so it cannot stall
local RAID_RALLY_DIST  = 700    -- rally point: this far from our base toward the enemy
local RAID_SWEEP_R     = 400    -- radius of the sweep around the enemy base
local RAID_ARRIVE_R    = 900
local RAID_STUCK_FRAMES = 900   -- a unit this old...
local RAID_STUCK_DIST   = 200   -- ...that has moved less than this from where it spawned is stuck

local CMD_GUARD = (CMD and CMD.GUARD) or 25

local raidUnits    = {}    -- [unitID] = true once sent, false while waiting at the rally point
local raidKind     = {}    -- [unitID] = "ground" | "escort"
local raidSpawn    = {}    -- [unitID] = { x, z, frame, checked }
local escortOf     = {}    -- [fighterID] = the Incisor it guards
...
-- Where new raiders wait: out past the factory's exit corridor, on the line to the enemy.
-- Waiting at the factory door would block the next unit coming out.
local function RaidRally()
    local bx, bz = RaidEnemyBase()
    local dx, dz = bx - baseX, bz - baseZ
    local len = math.sqrt(dx * dx + dz * dz)
    if len < 1 then return baseX, baseZ end
    local mapX, mapZ = Game.mapSizeX or 8192, Game.mapSizeZ or 8192
    local rx = math.max(MAP_MARGIN, math.min(mapX - MAP_MARGIN, baseX + dx / len * RAID_RALLY_DIST))
    local rz = math.max(MAP_MARGIN, math.min(mapZ - MAP_MARGIN, baseZ + dz / len * RAID_RALLY_DIST))
    return rx, rz
end

local function RaidSendToRally(uid)
    if not baseX then return end
    local rx, rz = RaidRally()
    spGiveOrderToUnit(uid, CMD_MOVE, {rx, spGetGroundHeight(rx, rz) or 0, rz}, {})
end
...
local function UpdateRaid(frame)
    ...
    local homeG, fieldG, homeE, fieldE = {}, {}, {}, {}
    for uid, sent in pairs(raidUnits) do
        local x, _, z = spGetUnitPosition(uid)
        if not x then
            raidUnits[uid] = nil
        elseif raidKind[uid] == "ground" then
            if sent then fieldG[#fieldG + 1] = uid else homeG[#homeG + 1] = uid end
        else
            if sent then fieldE[#fieldE + 1] = uid else homeE[#homeE + 1] = uid end
        end
        -- Stuck check: old enough, and still where it spawned.
        local sp = raidSpawn[uid]
        if x and sp and not sp.checked and raidKind[uid] == "ground"
           and frame - sp.frame >= RAID_STUCK_FRAMES then
            sp.checked = true
            if (x - sp.x) ^ 2 + (z - sp.z) ^ 2 < RAID_STUCK_DIST ^ 2 then
                Spring.Echo(string.format(
                    "[RAID] STUCK unit %d at (%d, %d) frame=%d: has not left its spawn point",
                    uid, x, z, frame))
            end
        end
    end
    ...
    if #homeG >= RAID_WAVE_GROUND or (firstWave and frame >= RAID_FORCE_FRAME and #homeG >= 2) then
        raidLaunched = true
        for _, uid in ipairs(homeG) do
            raidUnits[uid] = true
            fieldG[#fieldG + 1] = uid
            spGiveOrderToUnit(uid, CMD_FIGHT, {bx, spGetGroundHeight(bx, bz) or 0, bz}, {})
        end
        Spring.Echo(string.format("[RAID] %s frame=%d (%s) incisors=%d escorts=%d",
            firstWave and "launch" or "reinforce", frame, GameClock(frame), #homeG, #fieldE))
    end

    -- Escorts join as soon as there is something to guard: they are fast, so they catch up.
    if #fieldG > 0 then
        for _, uid in ipairs(homeE) do
            raidUnits[uid] = true
            fieldE[#fieldE + 1] = uid
        end
    end

    -- Each escort guards one Incisor; re-assign when its charge dies.
    local alive = {}
    for _, gid in ipairs(fieldG) do alive[gid] = true end
    local rr = 0
    for _, eid in ipairs(fieldE) do
        local g = escortOf[eid]
        if not (g and alive[g]) then
            if #fieldG > 0 then
                rr = rr % #fieldG + 1
                g = fieldG[rr]
                escortOf[eid] = g
                spGiveOrderToUnit(eid, CMD_GUARD, {g}, {})
            else
                escortOf[eid] = nil
                RaidSendToRally(eid)
                raidUnits[eid] = false   -- nothing to guard: wait for the next wave
            end
        end
    end
    ...
    -- Arrival, and the sweep: idle Incisors are handed the next point around the base.
    if not raidArrived then
        for _, uid in ipairs(fieldG) do
            local x, _, z = spGetUnitPosition(uid)
            if x and (x - bx) ^ 2 + (z - bz) ^ 2 < RAID_ARRIVE_R ^ 2 then
                raidArrived = true
                Spring.Echo(string.format("[RAID] arrived frame=%d (%s) incisors in field=%d",
                    frame, GameClock(frame), #fieldG))
                break
            end
        end
    end
    ...
    for _, uid in ipairs(fieldG) do
        if IsIdle(uid) then
            ...
            spGiveOrderToUnit(uid, CMD_FIGHT, {wx, spGetGroundHeight(wx, wz) or 0, wz}, {})
        end
    end
end
...
function widget:UnitFinished(unitID, unitDefID, teamID)
    ...
    if RAID_DEFS[d.name] then
        -- Wait at the rally point (not at the factory door) until a wave goes.
        raidUnits[unitID] = false
        raidKind[unitID]  = RAID_DEFS[d.name]
        local x, _, z = spGetUnitPosition(unitID)
        if x then raidSpawn[unitID] = { x = x, z = z, frame = Spring.GetGameFrame and Spring.GetGameFrame() or 0 } end
        RaidSendToRally(unitID)
