...
-- Commander blueprint deferred start (engine drops orders issued at frame 0).
local comBlueprintPending = false   -- true once StartComBlueprint sets up baseX/Z
local comBlueprintIssued  = false   -- true once QueueComBlueprint has actually run
...
function widget:GameFrame(frame)
    ...
    -- Issue commander blueprint once the engine is ready to accept orders.
    if comBlueprintPending and frame >= 15 then
    ...
    if comBlueprintIssued and frame < 450 and frame % 30 == 0 then
        if commanderID and spGetUnitDefID(commanderID) then
            local cmds = spGetUnitCommands(commanderID, 1)
            if not cmds or #cmds == 0 then
                if DEBUG then Spring.Echo("[WE] Commander idle in early game, retrying blueprint") end
                QueueComBlueprint(baseX, baseZ)
            end
        end
    end
