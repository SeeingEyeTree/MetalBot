...
--
-- 2026-09-18 change (run 20260918_024045, new_algorithm, paired with
-- unit_controller.lua's rez-bot logic): the T1 bot lab (corlab) isn't in
-- LAB_QUEUES, so it used the generic fallback below -- which excluded *every*
-- od.isBuilder buildOption, including cornecro (the Cortex Graverobber:
-- canresurrect=true, no buildOptions of its own). Rez bots were therefore never
-- built by anything. GetBuildCache now special-cases od.canResurrect into its
-- own bucket ahead of that exclusion, and the fallback queues one up to
-- REZ_BOT_TARGET, same pattern as the existing scout target.
...
local REZ_BOT_TARGET = 2  -- Graverobbers to keep alive/queued (heal + resurrect, see unit_controller.lua)
...
function widget:UnitFinished(unitID, unitDefID, teamID)
    ...
    if d.canResurrect and not myRezBots[unitID] then
        myRezBots[unitID] = true
        rezBotCount = rezBotCount + 1
        return
    end
...
function widget:UnitDestroyed(unitID)
    ...
    if myRezBots[unitID] then
        myRezBots[unitID] = nil
        rezBotCount = math.max(0, rezBotCount - 1)
    end
end
...
function widget:GameFrame(frame)
    ...
    local needRezBot = rezBotCount < REZ_BOT_TARGET
    ...
    for labID, labDefID in pairs(labs) do
        if spGetUnitDefID(labID) and QueueEmpty(labID) then
            ...
            if qdata then
                ...
                -- Generic fallback: scouts first, then a rez bot if under target,
                -- then cost-weighted combat pick.
                local cache = GetBuildCache(labDefID)

                if needScout and #cache.scouts > 0 then
                    choice = CheapestScout(cache.scouts)
                end
                if not choice and needRezBot and #cache.rez > 0 then
                    choice = cache.rez[1]
                end
