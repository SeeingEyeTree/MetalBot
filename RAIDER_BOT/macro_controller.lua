...
--
-- RAIDER_BOT differs from DRAGON_BOT in its kickstart: the blueprint
...
local airLabPromoted  = false -- the air lab item has been moved to the front of the queue
...
function widget:Initialize()
    ...
    local ok2, r2 = pcall(VFS.Include,
        "LuaUI/Widgets/blueprints/general/raider_blueprint.lua")
...
-- The air lab is one item in a ~90-item queue, and builders take the lowest-index item they
-- can build, so where it landed in time depended on how fast everything ahead of it went:
-- it finished at 3:53, 4:33 and 6:28 in three runs of the same blueprint.  Once the
-- opening nanos are up (the sim puts it right after nano #4) move it to the front, the
-- same way the old hand-off did.
--
-- That nano count is a proxy for "the kickstart is far enough along", and on a big or
-- slow-running kickstart it can arrive well after metal is ALREADY banking -- measured:
-- metal was banking before the nano gate opened, and nothing acted on it because
-- UpdateHandoff bailed out on `not airLabReady` before the banking check ever ran, so
-- the lab (and everything behind it) waited on the nano count alone.  Banking now
-- promotes the lab immediately, same as the nano gate, whichever comes first.
local AIR_LAB_AFTER_NANOS = 4
local function PromoteAirLab(frame, why)
    if airLabPromoted or not distState then return end
    for i, item in ipairs(distState.queue) do
        if item.n == "corap" then
            airLabPromoted = true
            ...
            Spring.Echo(string.format("[MC] air lab moved to the front of the queue frame=%d (%s)",
                frame, why))
            return
        end
    end
    -- No corap item in the blueprint at all (should not happen, but this is the one
    -- event that decides whether the bot scales -- a silent no-op here is silent
    -- failure for the whole game).  Fall back to placing one directly, the way
    -- DRAGON_BOT's hand-off used to.
    ...
    if not x then
        ...
        airLabPromoted = false   -- try again next tick
        return
    end
    ...
    if item then
        Spring.Echo(string.format(
            ...
            frame, tostring(name), x, z, nanos or 0, why))
    else
        airLabPromoted = false
        Spring.Echo("[MC] hand-off fallback: could not queue " .. tostring(name))
    end
end
...
local function UpdateHandoff(frame, resources)
    ...
    -- One banking measurement, used for whichever action is next: promoting the lab
    -- while it is still in the queue, or starting the grids once it exists.
    ...
    if not airLabPromoted then
        if BP_PLACER.CountBuilt(distState, "nano") >= AIR_LAB_AFTER_NANOS then
            PromoteAirLab(frame, "nano threshold")
            bankFrames = 0
        elseif bankFrames >= BANK_HOLD then
            PromoteAirLab(frame, "metal banking")
            bankFrames = 0
        end
        return
    end
