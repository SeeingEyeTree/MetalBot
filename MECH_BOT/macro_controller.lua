...
--
-- MECH_BOT: DRAGON_BOT's macro plus three unit-side additions (game_mechanics 1.4, 1.5,
-- 6.2): the commander is retired to a safe spot once the opening is done and dodges
-- nearby threats (bar_framework/commander_guard.lua); a bot lab is rebuilt outside the
-- grid lattice for rez bots; and the radar con bots go to the enemy-facing side.
-- Also fixes the "next factory becomes the bot lab" bug (lessons_learned 2026-09-21).
--
...
local botLabID    = nil
local botLabSeen  = false   -- only the kickstart's own lab is ever "the bot lab"
...
-- Commander safety (bar_framework/commander_guard.lua).
local CG          = nil
local comRemoved  = false   -- taken out of the build order by the guard
...
local function UpdateCommanderPhase(frame, resources)
    ...
    -- The commander guard has it (retired, or dodging a threat): hands off.
    if comRemoved then return end
...
local function DispatchConBots()
    ...
    local radarCentre = mb and mb.axisX and math.atan2(mb.axisZ, mb.axisX) or nil
    local sent = 0
    for i, cid in ipairs(conBots) do
        if spGetUnitDefID(cid) then
            ...
            for r = reserve and 1e9 or 1600, 3200, 400 do   -- reserved: skip the search
                for a = 0, 7 do
                    -- Radar is warning time, so it goes on the side the enemy comes
                    -- from (game_mechanics 6.2), spread across that front; the old
                    -- full circle put half of it behind the base.
                    local ang = (a + i * 0.5) * math.pi / 4
                    if radarCentre then
                        local k = (a % 2 == 0) and (a / 2) or -((a + 1) / 2)
                        ang = radarCentre + (i - 1.5) * 0.45 + k * 0.35
                    end
...
-- ── Bot lab for rez bots ─────────────────────────────────────────────────────

-- game_mechanics 1.4 calls Graverobbers a required unit, and cornecro only comes out of
-- a T1 bot lab -- which the kickstart reclaims.  So once the vehicle plant stands, a new
-- bot lab goes up for the rez bots (the lab controller keeps their quota).
--
-- WHERE matters more than when: the mex lattice is a wall to ground units (lessons
-- 2026-09-24: the vehicle plant was boxed in by ~9:00).  The lab is placed OUTSIDE every
-- grid cell assigned so far, preferring the side facing the enemy (a shorter walk to the
-- front), facing the enemy, and the cells it and its exit sit in are then reserved so no
-- grid is ever placed over them.
-- Wrapped in do ... end: the main chunk is at Lua 5.1's 200-local limit, and locals
-- inside a block stop counting once it closes.  Only MaybeQueueRezLab escapes.
local MaybeQueueRezLab
do
local rezLabDefID      = nil
local rezLabQueued     = false
local rezLabOrderFrame = nil
local rezLabBuilder    = nil
local rezLabBuilt      = false
local REZ_LAB_NAME     = "corlab"
local REZ_LAB_WATCHDOG = 2400   -- ordered but no lab this long later: try again
local REZ_LAB_MIN_R    = 900
local REZ_LAB_MAX_R    = 3000
...
-- Is (x, z) with this half-extent clear of every grid cell claimed so far?
local function OutsideGrids(x, z, half)
    local reach = GRID_SPACING / 2 + half + 48
    for key in pairs(assignedAnchors) do
        local ax, az = ParseKey(key)
        if ax and math.abs(x - ax) < reach and math.abs(z - az) < reach then return false end
    end
    return true
end

local function FacingToward(dx, dz)
    if math.abs(dx) > math.abs(dz) then return dx > 0 and 1 or 3 end
    return dz > 0 and 0 or 2
end

local function FindRezLabSpot()
    local half = HalfExtent(rezLabDefID)
    local mb = WG and WG.MetalBot
    local ax, az = (mb and mb.axisX) or 1, (mb and mb.axisZ) or 0
    local centre = math.atan2(az, ax)
    local mapX, mapZ = Game.mapSizeX or 12288, Game.mapSizeZ or 12288
    local best, bestScore = nil, math.huge
    for r = REZ_LAB_MIN_R, REZ_LAB_MAX_R, 150 do
        for a = 0, 15 do
            local ang = centre + (a - 7.5) * math.pi / 8
            local x, z = BP_PLACER.SnapToBuildGrid(rezLabDefID,
                             baseX + r * math.cos(ang), baseZ + r * math.sin(ang))
            if x > 300 and z > 300 and x < mapX - 300 and z < mapZ - 300 then
                -- Enemy-facing first, then close: 400 elmos of extra walk per radian.
                local off = math.abs(((ang - centre + math.pi) % (2 * math.pi)) - math.pi)
                local score = r + off * 400
                if score < bestScore and OutsideGrids(x, z, half)
                   and not ClashesWithBuildOrder(x, z, half) then
                    local y  = spGetGroundHeight(x, z) or 0
                    local facing = FacingToward(ax, az)
                    local ok = Spring.TestBuildOrder(rezLabDefID, x, y, z, facing)
                    if ok and ok ~= 0 then best, bestScore = { x, z, facing }, score end
                end
            end
        end
    end
...
-- Keep grids off the lab's cell and off a lane of cells in front of its door: the
-- lattice keeps growing, and one free cell ahead would soon be walled in as well.
local REZ_LANE_CELLS = 3
local function ReserveAround(x, z, facing)
    local exits = { [0] = { 0, 1 }, [1] = { 1, 0 }, [2] = { 0, -1 }, [3] = { -1, 0 } }
    local e = exits[facing] or { 0, 1 }
    local n = 0
    for k = 0, REZ_LANE_CELLS do
        local px, pz = x + e[1] * GRID_SPACING * k, z + e[2] * GRID_SPACING * k
        local ax = baseX + math.floor((px - baseX) / GRID_SPACING + 0.5) * GRID_SPACING
        local az = baseZ + math.floor((pz - baseZ) / GRID_SPACING + 0.5) * GRID_SPACING
        local key = AnchorKey(ax, az)
        if not assignedAnchors[key] then assignedAnchors[key] = true; n = n + 1 end
    end
    return n
end

local function VehiclePlantFinished()
    for _, uid in ipairs((Spring.GetTeamUnitsByDefs and vpDefID
                          and Spring.GetTeamUnitsByDefs(myTeamID, vpDefID)) or {}) do
        if not Spring.GetUnitIsBeingBuilt(uid) then return true end
    end
    return false
end

local function RezLabBuilder()
    if vpBuilder and spGetUnitDefID(vpBuilder) and CanBuild(vpBuilder, rezLabDefID) then
        return vpBuilder, false
    end
    for i, cid in ipairs(freeAirCons) do
        if spGetUnitDefID(cid) and CanBuild(cid, rezLabDefID) then
            table.remove(freeAirCons, i)
            return cid, true
        end
    end
    return nil
...
MaybeQueueRezLab = function(frame)
    if rezLabBuilt or not vpBuilt or not baseX or not GRID_SPACING then return end
    rezLabDefID = rezLabDefID or (UnitDefNames[REZ_LAB_NAME] and UnitDefNames[REZ_LAB_NAME].id)
    if not rezLabDefID then return end

    if rezLabQueued then
        for _, uid in ipairs((Spring.GetTeamUnitsByDefs
                              and Spring.GetTeamUnitsByDefs(myTeamID, rezLabDefID)) or {}) do
            if uid ~= botLabID then
                rezLabBuilt = true
                Spring.Echo(string.format("[MC] REZ LAB frame=%d: %s up (unit %d)",
                    frame, REZ_LAB_NAME, uid))
                -- An air con borrowed for this goes back to the grid pool.
                if rezLabBuilder and rezLabBuilder ~= vpBuilder and spGetUnitDefID(rezLabBuilder) then
                    freeAirCons[#freeAirCons + 1] = rezLabBuilder
                end
                return
            end
        end
        if frame - rezLabOrderFrame > REZ_LAB_WATCHDOG then
            Spring.Echo(string.format("[MC] rez lab: none %d frames after ordering, retrying",
                frame - rezLabOrderFrame))
            if rezLabBuilder and rezLabBuilder ~= vpBuilder and spGetUnitDefID(rezLabBuilder) then
                freeAirCons[#freeAirCons + 1] = rezLabBuilder
            end
            rezLabQueued, rezLabBuilder = false, nil
        end
        return
    end

    -- The con bot that built the plant must finish it first: leaving a nanoframe
    -- decays it.
    if not VehiclePlantFinished() then return end
    local x, z, facing = FindRezLabSpot()
    if not x then return end
    local builder, borrowed = RezLabBuilder()
    if not builder then return end
    local y = spGetGroundHeight(x, z) or 0
    spGiveOrderToUnit(builder, -rezLabDefID, { x, y, z, facing }, {})
    rezLabQueued, rezLabOrderFrame, rezLabBuilder = true, frame, builder
    local reserved = ReserveAround(x, z, facing)
    Spring.Echo(string.format(
        "[MC] REZ LAB frame=%d via %s: %s at (%d, %d) facing %d, %d grid cells reserved",
        frame, borrowed and "air con" or "reserved con bot", REZ_LAB_NAME, x, z, facing, reserved))
end
end   -- do (rez lab)

-- ── Commander safety ─────────────────────────────────────────────────────────

-- Retire the commander once the kickstart is finished and the grids have taken over,
-- or at COM_RETIRE_FRAME regardless: game_mechanics 1.5, it only matters in the
-- opening.  The guard can also take it at any moment to dodge a threat; when it hands
-- the commander back (threat over, not yet retired) it rejoins the build order.
local UpdateCommanderSafety
do
local COM_RETIRE_FRAME = 8 * 60 * 30

UpdateCommanderSafety = function(frame, resources)
    if not CG or not commanderID or not spGetUnitDefID(commanderID) then return end
    local mb = (WG and WG.MetalBot) or {}
    local retire = frame >= COM_RETIRE_FRAME
        or (airLabID ~= nil and gridsAssigned > 0 and distState.done)
    local state = CG.Update(frame, commanderID, resources, {
        retire = retire, homeX = baseX, homeZ = baseZ,
        foeX = mb.foeX, foeZ = mb.foeZ, threats = mb.threats,
    })
    if state ~= "free" then
        if not comRemoved then
            comRemoved = true
            -- No CMD_STOP here: the guard has just given its own order this tick.
            comGuardTarget = nil
            BP_PLACER.RemoveBuilder(distState, commanderID)
        end
    elseif comRemoved then
        comRemoved = false
        BP_PLACER.AddBuilder(distState, commanderID)
    end
end
end   -- do (commander safety)
...
function widget:UnitCreated(unitID, unitDefID, teamID, builderID)
    ...
    if d.isFactory then
        ...
        elseif not botLabID and not botLabSeen then
            -- Only the kickstart's lab.  Once it is reclaimed botLabID is nil again, and
            -- the next factory (the vehicle plant, the rez lab) used to be adopted as
            -- "the bot lab" and made to build a con as if it were one.
            botLabID, botLabSeen = unitID, true
...
function widget:GameFrame(frame)
    ...
    -- Before the commander phase and the placer: a commander the guard has taken must
    -- be out of the builder pool before anything hands it a claim.
    UpdateCommanderSafety(frame, resources)
