...
So each condition is repeated, and a bot is only called better when its runs do not
overlap the other's at all in a slot (min of one > max of the other). That is a
deliberately conservative, non-parametric bar: with small samples it is much easier to
defend than a mean comparison, and it fails loudly rather than quietly.
...
DEFAULT_FRAME = {"income": 9000, "army": 14400}
...
def main() -> int:
    ...
    ap.add_argument("--repeat", type=int, default=3,
                    help="matches per slot order (default 3; 1 is not enough to conclude)")
    ap.add_argument("--metric", choices=("army", "income"), default="army",
    ...
    print(f"repeat = {args.repeat} ({2 * args.repeat} matches, ~{12 * args.repeat} min)\n",
          flush=True)
    ...
    print(f"\n  slot 0: {label[s0]}")
    print(f"  slot 1: {label[s1]}")

    spreads = [max(v) / min(v) for v in score.values() if len(v) > 1 and min(v) > 0]
    if spreads:
        note = f" (expected ~{floor:.2f}x)" if floor else ""
        print(f"\n  worst within-condition spread this run: {max(spreads):.2f}x{note}")

    print()
    if s0 == 1 and s1 == 1:
        print("VERDICT: A is better -- ahead in both slots with no overlap.")
        return 0
    if s0 == -1 and s1 == -1:
        print("VERDICT: B is better -- ahead in both slots with no overlap.")
        return 1
    print("VERDICT: NO DIFFERENCE DEMONSTRATED.")
    print("The runs overlap in at least one slot, so this evidence cannot distinguish the")
    print("two bots. Do NOT record this as a win or a regression. Either raise --repeat, or")
    print("accept that the change is smaller than this harness can measure.")
    return 2
